...
window.onload = function init()
{
    ...
    canvas.addEventListener("mousedown", function(e) {        
        ...
        if (mode === 1) {
            ...
            triangleArray.push(point);
            if (triangleIndex%3 === 0 && triangleArray.length > 0) {
                var v3 = triangleArray[triangleArray.length-1];
                var v2 = triangleArray[triangleArray.length-2];
                var v1 = triangleArray[triangleArray.length-3]; 
            ...
            circleArray.push(point);
            if (circleIndex%2 === 0 && circleArray.length > 0) {
                circleCount++;

                midPoint = circleArray[circleArray.length-2];
                outPoint = circleArray[circleArray.length-1];
